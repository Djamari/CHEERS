...
import numpy as np
from ROIs.main4_match import get_partial_correlation, get_max_correlation, get_annotations_matrix
...
def main_analysis_correlation(searchlights):
    ...
    for analysis_label in analyses.keys():
        ...
        for SL_idx, voxel_indices in enumerate(tqdm(searchlights)):
            ...
            p = multiprocessing.Process(target=get_partial_correlation_and_save, args=(strengths_allRuns, annotation_matrix, save_prefix))
            processes.append(p)
            p.start()
...
if __name__ == '__main__':
    ...
    maxRs_SLs_allRuns = np.zeros((len(searchlights), len(analyses.keys())))
    ...
    for label_idx, analysis_label in enumerate(analyses.keys()):  # One figure per label
        for SL_idx, voxel_indices in enumerate(tqdm(searchlights)):
            ...
            maxRs = np.load(save_intermediate_results + "analysis_" + analysis_label +  "SL" + str(SL_idx) + "_maxRs.npy")
        ...
        nib.save(map_nifti, savedir + 'analysis_' + analysis_label + '_Rs_relative.nii')

        # Threshold at p<0.05
        map_nifti = nib.Nifti1Image(mean_ratios_thres_uncorrected, affine)
        nib.save(map_nifti, savedir +  'analysis_' + analysis_label + '_Rs_relative_thresholded_uncorrected.nii')

        # Threshold based on correction
        map_nifti = nib.Nifti1Image(mean_ratios_thres_corrected, affine)
        nib.save(map_nifti, savedir +  'analysis_' + analysis_label + '_Rs_relative_' + '_thresholded_corrected.nii')
